...
def generate_beam(
    ...
    with paddle.no_grad():
        ...
        for i in range(entry_length):
            ...
            is_stopped = paddle.cast(is_stopped, 'int32') + paddle.cast(next_tokens.equal(stop_token_index).squeeze(), 'int32')
        ...
        for entry_idx in range(entry_count):
            ...
            for i in range(entry_length):
                ...
                cumulative_probs = paddle.cumsum(
                    F.softmax(sorted_logits, axis=-1), axis=-1
                )
...
class CaptionDecoder(object):
    def __init__(self, pretrained_path, hidden_dim=-1):
        if hidden_dim < 0:
            hidden_dim = None
